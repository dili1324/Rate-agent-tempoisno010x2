...
from rate_agent.rate_client import RateClient, RateDataError, parse_currency_quote, parse_gold_quote
...
def test_rate_client_calls_alpha_vantage_endpoints() -> None:
    ...
    assert tempo.calls[0][0].endswith("/alphavantage/commodity-price")
    assert tempo.calls[0][1]["commodity"] == "GOLD_SILVER_SPOT"
    assert tempo.calls[0][1]["symbol"] == "XAU"
